...
def get_hfi_returns():
    ...
    """
    hfi = pd.read_csv("data/edhec-hedgefundindices.csv",header=0,index_col=0,na_values=-99.99,parse_dates=True)
    hfi = hfi/100
...
def skewness(r):
    ...
    """
    demeaned_r = r - r.mean() # use population stadard deviation, so sed dof=0
...
def optimal_weights(n_points,er,cov):
    """
    Generates list of weights to run the optimizer to minimize the vol
    """
...
def plot_ef(n_points, er, cov,show_cml=False,style=".-", riskfree_rate=0, show_ew = False, show_gmw=False):
    """
    ...
    if show_gmw:
        ...
        # display EW
        ax.plot([vol_gmw],[r_gmw],color="midnightblue",marker="o",markersize="8")
    if show_cml:
        ...
        ax.plot(cml_x, cml_y, color="red", marker = "o", linestyle = "dashed",markersize=4,linewidth=2)
    return ax
